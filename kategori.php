...
<?php 
    $title = "Kategori: " . $_GET['terim'] . $kisaltma;
...
<?php 
    ...
    //--------------------------------------------------
    $icerikSayisiSql = "SELECT COUNT(*) FROM icerik WHERE kategori  LIKE '%$search%'";
    ...
    //--------------------------------------------------
    $sql ="SELECT * FROM icerik WHERE kategori  LIKE '%$search%' LIMIT '$sayfaBasiIcerik' OFFSET '$offset'";
    $sira = $offset;
    $sonuc = $db->query($sql);
