...
<?php 
    $title = "Etiket: " . $_GET['terim'] . $kisaltma;
...
<?php 
    ...
    //--------------------------------------------------
    $icerikSayisiSql = "SELECT COUNT(*) FROM icerik WHERE etiket LIKE '%$search%'";
    ...
    //--------------------------------------------------
    $sql ="SELECT * FROM icerik WHERE etiket LIKE '%$search%' LIMIT '$sayfaBasiIcerik' OFFSET '$offset'";
    $sira = $offset;
    $sonuc = $db->query($sql);
